...
router.post('/:id/approve', authenticate, async (req, res) => {
    try {
      ...
      if (request.action === 'put') {
        const eventToUpdate = await Event.findByPk(request.eventId);
        if (!eventToUpdate) {
          return res.status(404).json({ message: 'Оригинальное событие не найдено' });
        }
      
        eventToUpdate.title = request.title;
        eventToUpdate.startTime = request.startTime;
        eventToUpdate.endTime = request.endTime;
        eventToUpdate.location = request.location;
        eventToUpdate.type = request.type;
        eventToUpdate.class = request.class;
        eventToUpdate.userIds = JSON.stringify(request.userIds);
